...
namespace Snowstorm::CVars
{
	...
	// Image path; when set, the bare runtime hosts a SpriteTest game module that spawns two overlapping
	// test sprites from it on the 2D canvas. A test hook: no shipped scene carries a sprite yet, so this is
	// how the sprite pass gets exercised headlessly (with quality.capture.frames) and its canvas orientation
	// checked against a known image.
