...
namespace Snowstorm::CVars
{
	...
	CVar<bool> QualityCaptureExact{"quality.capture.exact", false, "Capture exactly quality.capture.frames after streaming completes, ignoring the convergence epsilon. For techniques that never reach it (RT GI/AO/TAA keep a permanent per-frame noise floor) the alternative is the safety cap, which fires on the next 16-frame convergence checkpoint; those checkpoints are phased from the frame streaming happened to finish, so the captured image drifts by a frame between runs and an unconverged image drifts with it. Measured on Sponza: repeats of one config spanned 0.026 FLIP without this, 0.0003 with it.", CVarFlags::ReadOnly};
	...
	CVar<std::string> QualityCaptureAtPathFrames{"quality.capture.at_path_frames", "", "Motion capture: ascending, comma-separated camera-route frame indices to capture, e.g. \"120,121,420,421\". Each is written as <quality.capture.path>_f<NNNNNN>_ldr.npy, plus a _poses.json manifest of the pose each was taken at. Empty = the single converged capture instead. Needs camera.path on; a route frame is the only clock a separate reference run can reproduce, since a moving image has no converged fixed point to detect. Pairs of adjacent frames are the usual request, because a temporal metric compares consecutive frames.", CVarFlags::ReadOnly};
	...
	CVar<bool> GiReSTIR{"render.gi.restir", false, "EXPERIMENTAL, off by default and not a shipping path. Resample the GI hemisphere samples (ReSTIR) instead of averaging them, keeping one sample plus its unbiased contribution weight and reusing it temporally and spatially (off = the cosine-weighted average). Off because it MEASURES WORSE than the average it replaces, at every capture window from 6 frames to converged: resampling alone is neutral (it is unbiased), and each form of reuse costs a little more, worst on the gallery viewpoint (FLIP 0.1747 -> 0.1805 at 6 frames, 0.1781 -> 0.1795 converged, against a 0.0002 noise floor). A reservoir carries ONE sample whose variance does not fall with M, and render.gi.rays averages 2 fresh ones per frame into a tuned SVGF chain, which is a stronger baseline than the 1-spp path trace ReSTIR is usually posed against. GPU cost is UNMEASURED and structurally additive (three reservoir targets written per frame plus a second dispatch). STARTUP-ONLY: it selects a shader permutation (SS_GI_RESTIR), because compiling the reservoir path in unconditionally cost the default path 22 VGPRs and two waves of occupancy for a feature that is off", CVarFlags::ReadOnly};
	CVar<bool> GiReSTIRTemporal{"render.gi.restir.temporal", true, "EXPERIMENTAL (under render.gi.restir). Reuse the previous frame's GI reservoir (requires render.gi.restir). Off isolates single-frame resampling from temporal reuse, which are separate sources of error", CVarFlags::Persist};
	CVar<bool> GiReSTIRSpatial{"render.gi.restir.spatial", true, "EXPERIMENTAL (under render.gi.restir). Combine each GI reservoir with its neighbours' (requires render.gi.restir). This is what turns a per-pixel single sample back into an effectively-many-sample estimate; resampling and temporal reuse alone measure worse than averaging", CVarFlags::Persist};
	CVar<float> GiReSTIRSpatialRadius{"render.gi.restir.spatial.radius", 16.0f, "EXPERIMENTAL (under render.gi.restir). Neighbour search radius for GI spatial reuse, in half-res pixels. Larger decorrelates more but rejects more neighbours on curved geometry", CVarFlags::Persist};
	CVar<int> GiReSTIRSpatialCount{"render.gi.restir.spatial.count", 4, "EXPERIMENTAL (under render.gi.restir). Neighbours sampled per pixel for GI spatial reuse", CVarFlags::Persist};
	CVar<bool> GiReSTIRSpatialVisibility{"render.gi.restir.spatial.visibility", false, "EXPERIMENTAL (under render.gi.restir). Trace a reconnection ray before accepting a reused neighbour sample. Matching depth and normal prove a shared surface, not an unoccluded path, so without this the reuse leaks light around occluders. Off by default: this traces opaque-only, so alpha-masked geometry over-occludes, and the two errors measure as a wash on Sponza while costing a ray per accepted neighbour", CVarFlags::Persist};
	...
	CVar<float> TaaDepthReject{"render.taa.depth_reject", 0.0f, "TAA depth-disocclusion rejection threshold: reject reprojected history whose linear depth differs by more than this fraction of view-space depth (kills ghost trails on disoccluded silhouettes). 0 = off (#127). With render.taa.depth_reject.slope on, this is the small BASE fraction added on top of the surface-slope allowance. DEFAULT 0 (OFF), measured: swept 0..0.08 against ColorVideoVDP on the animated scene, 0 wins by 0.0090 JOD and every non-zero value is indistinguishable from every other (0.0007 span). A step rather than a curve, so this is not a threshold wanting a better value: any depth-based rejection costs quality here, and the neighbourhood clamp already handles disocclusion (the strategy INSIDE moved to for the same reason). A static viewpoint has no disocclusion, so only the motion gate can judge this.", CVarFlags::Persist};
	...
	CVar<float> ShadowScale{"render.shadows.scale", 1.0f, "Stochastic RT shadow internal resolution: the shadow-ratio trace runs at this fraction of viewport res, then a depth-aware bilateral upsample restores full res. DEFAULT 1.0 (FULL-RES): shadows are the highest-frequency signal in the frame (sharp contact/thin shadows), and measured on the motion gate every spatial metric degrades monotonically as this drops (JOD 3.1737 at 1.0, 3.1707 at 0.75, 3.1560 at 0.5), with only tFLIP improving, which is the blur signature. The counterweight is the cost: 0.5 measures 0.444 ms over rt-off against 1.659 ms at full res, so it buys back 1.215 ms for 0.0177 JOD, under 2% of a just-noticeable difference. NOT a settled choice, and note that UE's MegaLights defaults r.MegaLights.DownsampleFactor to 2, i.e. it samples and denoises at QUARTER res, which is what earns a stochastic aggregate the right to downsample where a per-light binary mask could not. Clamped to [0.25, 1.0]. Clamped to [0.25, 1.0].", CVarFlags::Persist};
	...
	CVar<int> ShadowDenoiseIterations{"render.shadows.denoise.iterations", 3, "RT shadow denoiser a-trous pass count: each doubles the tap stride (1,2,4,...) for a wider edge-aware blur. Runs ONLY under the stochastic producer, so at the shipping default (render.shadows.stochastic off) this chain is not dispatched and its cost is zero. Kept at 3 because the stochastic path visibly needs it: artifacts where two lights overlap disappear when it is on. It was briefly defaulted to 0 on the strength of the motion gate scoring 0 better on every metric, which was wrong because that gate's route never renders the lit side aisles where the artifacts are (#169). Clamped to [0, 5]", CVarFlags::Persist};
	...
	CVar<bool> ShadowStochastic{"render.shadows.stochastic", false, "EXPERIMENTAL. RT shadow technique within mode 2 (Ray Traced). OFF (default, and the supported path) = per-light INLINE RT shadows, one sharp ray per light in DefaultLit. ON = the STOCHASTIC aggregate-ratio pass (MegaLights-lite): importance-sample one light per pixel, trace one ray, denoise. Constant cost in light count where inline grows per light, so it is the many-light technique in principle, and it measures more temporally stable (tFLIP 0.0131 against inline 0.0154). It is NOT production-ready here: it shows artifacts under motion where two lights overlap, which is consistent with per-pixel light selection differing between neighbours, and it depends on the shadow a-trous to hide them (#170). Inline is the default until that is fixed. Prefer ShadowStochasticActive() over reading this directly", CVarFlags::Persist};
	...
	CVar<float> ShadowSunAngleDeg{"render.shadow.sun_angle_deg", 0.53f, "Sun angular DIAMETER in degrees (halved to a cone half-angle): it drives RT soft-shadow penumbra width for the directional light, larger = softer. Default is the sun's physical diameter. Widening it costs noise as well as softness on the inline path, which takes 2 rays per light and so quantizes penumbra visibility to {0, 0.5, 1}: the penumbra band is d*tan(half-angle) wide, and a caster thinner than that band never reaches umbra, leaving its whole shadow in the noisy region. Only used by the RT soft path.", CVarFlags::Persist};
	...
	CVar<bool> RTHitLights{"render.rt.hit_lights", true, "Shade RT GI secondary hits with the scene's point/spot lights in addition to the sun, closing the bias against the reference path tracer's full NEE. Picks ONE light per hit in proportion to its unshadowed contribution and traces one extra shadow ray (the UE5 MegaLights estimator), so the cost is constant in the light count. Measured on Sponza: +0.7 to +0.9 dB PSNR vs the path tracer for ~+40% on the GI pass. GI only, since the same estimator in the reflection pass measured no gain at twice the cost.", CVarFlags::Persist};
	...
	CVar<int> ReflectionDenoiseIterations{"render.reflections.denoise.iterations", 0, "RT reflection denoiser à-trous pass count (#129 Inc 3a): each pass doubles the tap stride (1,2,4,…) for a wider edge-aware blur. More = smoother but costlier / more over-blur on glossy detail. Clamped to [0, 5]; 0 disables like render.reflections.denoise off. DEFAULT 0 (OFF), measured: the shared a-trous is guided by the RECEIVER's normal and depth, which are constant across a flat reflective surface, so its kernel is wide open exactly where reflected detail lives. It makes the pixels it touches worse (error 23.327 -> 24.103 inside its own footprint) and costs 1.115 ms, 13.5% of the +refl frame, being full-res with no upsample after it. Turning it off improves FLIP, PSNR, SSIM and JOD together, and is confirmed by eye as indistinguishable.", CVarFlags::Persist};

	CVar<float> ReflectionDenoiseHitDist{"render.reflections.denoise.hitdist", 0.0f, "RT reflection hit-distance edge-stop phi (NRD REBLUR-style), in NORMALIZED units: the guide is the raw trace's per-pixel mean hit distance divided by render.reflections.range, so this number means the same thing as render.ao.denoise.hitdist rather than an inverse-world-units figure. Weights a-trous taps by |delta hit distance| so reflected content at a different depth is not blurred together. This is the one edge-stop that can see a reflection edge at all: the shared a-trous is guided by the RECEIVER's normal and depth, which are constant across a flat reflective surface, so its kernel is wide open exactly where reflected detail lives. DEFAULT 0 (OFF), which is bit-identical to the previous behaviour. Reflections trace 1 ray/pixel by default, so the raw guide is noisy between neighbours and too large a phi rejects every tap, turning the filter into a no-op.", CVarFlags::Persist};
	...
	CVar<float> GIRange{"render.gi.range", 17.0f, "Indirect gather distance in world units, read by both GI producers: RT GI sets ray.TMax to it, and SSGI derives its march step and depth-acceptance thickness by dividing it across a fixed 32 steps. RayQuery cannot distinguish leaving the scene from exhausting TMax, so both land in the sky-cube miss branch: too short a range feeds un-occluded sky to rays that died in mid-air inside a closed interior. Default 17 measured to improve FLIP on all three viewpoints in isolated rtgi (mean -9.6%, +0.67 dB PSNR); quality is flat between 17 and 24.", CVarFlags::Persist};
