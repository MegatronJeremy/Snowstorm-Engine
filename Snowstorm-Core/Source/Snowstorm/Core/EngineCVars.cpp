...
namespace Snowstorm::CVars
{
	...
	CVar<bool> GiReSTIR{"render.gi.restir", false, "Resample the GI hemisphere samples (ReSTIR) instead of averaging them, keeping one sample plus its unbiased contribution weight and reusing it temporally and spatially (off = the cosine-weighted average). Off because it MEASURES WORSE than the average it replaces, at every capture window from 6 frames to converged: resampling alone is neutral (it is unbiased), and each form of reuse costs a little more, worst on the gallery viewpoint (FLIP 0.1747 -> 0.1805 at 6 frames, 0.1781 -> 0.1795 converged, against a 0.0002 noise floor). A reservoir carries ONE sample whose variance does not fall with M, and render.gi.rays averages 2 fresh ones per frame into a tuned SVGF chain, which is a stronger baseline than the 1-spp path trace ReSTIR is usually posed against. GPU cost is UNMEASURED and structurally additive (three reservoir targets written per frame plus a second dispatch)", CVarFlags::Persist};
	CVar<bool> GiReSTIRTemporal{"render.gi.restir.temporal", true, "Reuse the previous frame's GI reservoir (requires render.gi.restir). Off isolates single-frame resampling from temporal reuse, which are separate sources of error", CVarFlags::Persist};
	CVar<bool> GiReSTIRSpatial{"render.gi.restir.spatial", true, "Combine each GI reservoir with its neighbours' (requires render.gi.restir). This is what turns a per-pixel single sample back into an effectively-many-sample estimate; resampling and temporal reuse alone measure worse than averaging", CVarFlags::Persist};
	CVar<float> GiReSTIRSpatialRadius{"render.gi.restir.spatial.radius", 16.0f, "Neighbour search radius for GI spatial reuse, in half-res pixels. Larger decorrelates more but rejects more neighbours on curved geometry", CVarFlags::Persist};
	CVar<int> GiReSTIRSpatialCount{"render.gi.restir.spatial.count", 4, "Neighbours sampled per pixel for GI spatial reuse", CVarFlags::Persist};
	CVar<bool> GiReSTIRSpatialVisibility{"render.gi.restir.spatial.visibility", false, "Trace a reconnection ray before accepting a reused neighbour sample. Matching depth and normal prove a shared surface, not an unoccluded path, so without this the reuse leaks light around occluders. Off by default: this traces opaque-only, so alpha-masked geometry over-occludes, and the two errors measure as a wash on Sponza while costing a ray per accepted neighbour", CVarFlags::Persist};
