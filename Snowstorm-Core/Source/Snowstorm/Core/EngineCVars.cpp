...
namespace Snowstorm::CVars
{
	...
	CVar<int> ShadowDenoiseIterations{"render.shadows.denoise.iterations", 0, "RT shadow denoiser a-trous pass count: each doubles the tap stride (1,2,4,...) for a wider edge-aware blur. More = smoother but costlier. Clamped to [0, 5]; 0 disables like render.shadows.denoise off. DEFAULT 0 (OFF), measured: on static viewpoints all four of FLIP/PSNR/SSIM/JOD degrade monotonically as iterations rise, and under motion JOD does the same (2.9401 at 0 against 2.8825 at 5). Unlike GI's, object disocclusion does not rescue it. Disabling it saves a measured 1.246 ms, 45% of what the stochastic producer costs over rt-off.", CVarFlags::Persist};
