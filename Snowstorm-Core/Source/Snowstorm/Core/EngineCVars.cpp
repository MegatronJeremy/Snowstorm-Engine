...
namespace Snowstorm::CVars
{
	...
	CVar<std::string> SpriteTest{"debug.sprite_test", "", "Image path; when set, the bare runtime hosts a SpriteTest game module that spawns two overlapping test sprites from it on the 2D canvas (one opaque at the top-left, one tinted half-transparent offset over it) so the sprite pass can be verified headlessly with quality.capture.frames", CVarFlags::ReadOnly};
