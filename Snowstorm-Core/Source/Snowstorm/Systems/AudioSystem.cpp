...
namespace Snowstorm
{
	...
	void AudioSystem::UpdateListener(AudioService& audio)
	{
		...
		m_HasListener = listener != entt::null;
		if (!m_HasListener)
		{
			// Not reported here: a scene with no spatial sound does not need an ear, and saying so would
			// warn every scene that happens to be silent. The source loop complains if one actually needs it.
			return;
		}
	...
	void AudioSystem::Execute(Timestep)
	{
		...
		for (const auto view = View<AudioSourceComponent>(); const entt::entity entity : view)
		{
			...
			if (source.Spatial)
			{
				// Now something does depend on the ear. Said once, not per frame.
				if (!m_HasListener && !m_WarnedNoListener)
				{
					m_WarnedNoListener = true;
					SS_CORE_WARN("Audio: a spatial AudioSource is playing but no entity has an "
					             "AudioListenerComponent; it is panned against the world origin. "
					             "Add one to the camera.");
				}
