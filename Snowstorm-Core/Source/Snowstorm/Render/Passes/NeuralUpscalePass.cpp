...
namespace Snowstorm
{
	...
	{
		if (m_Width == w && m_Height == h && m_Output)
		{
			return;
		}
		...
		m_FeatureBase = Buffer::Create(featureBytes, BufferUsage::Storage, nullptr, false, "NeuralFeatureBase");
		m_FeatureA = Buffer::Create(featureBytes, BufferUsage::Storage, nullptr, false, "NeuralFeatureA");
		m_FeatureB = Buffer::Create(featureBytes, BufferUsage::Storage, nullptr, false, "NeuralFeatureB");
		...
		m_Output = Texture::Create(od);
		m_OutputView = m_Output->GetDefaultView();
	}
	...
	{
		if (!ctx || !lowResColor || outWidth == 0 || outHeight == 0 || !m_Output)
		{
		...
		// ---- Stage 1: bilinear-upsample LR into FeatureBase (the residual skip connection, channels 0..2) ----
		{
			...
			set->SetBuffer(2, {.Buffer = m_FeatureBase, .Offset = 0, .Range = featureBytes});
		...
		Ref<Buffer> cur = m_FeatureBase;
		Ref<Buffer> next = m_FeatureA;
		Ref<Buffer> other = m_FeatureB; // the free scratch not currently cur/next
		for (size_t i = 0; i < m_Model.Layers.size(); ++i)
		{
			...
			next = (cur == m_FeatureBase) ? other : cur; // free scratch to write next
			cur = producedInto;
		}
		...
		ctx->TransitionToStorage(m_Output);
		{
			SizeCB cb{};
			cb.OutSize = outSize;
			...
			set->SetBuffer(0, {.Buffer = m_FeatureBase, .Offset = 0, .Range = featureBytes});
			set->SetBuffer(1, {.Buffer = cur, .Offset = 0, .Range = featureBytes});
			set->SetTexture(2, m_OutputView);
			set->SetBuffer(3, {.Buffer = ubo, .Offset = 0, .Range = sizeof(SizeCB)});
			set->Commit();
		...
		ctx->TransitionToSampled(m_Output);
	}
}
