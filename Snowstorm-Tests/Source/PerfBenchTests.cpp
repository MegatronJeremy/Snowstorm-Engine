...
namespace
{
	// Substring check — the JSON is small + deterministic, so asserting on exact key/value fragments is
...
TEST_CASE("PerfBench averages per-pass ms over frames", "[render][perfbench]")
{
	...
	const std::string json = acc.ToJson("TestGPU", "rt-off", true);
...
TEST_CASE("PerfBench keys are sorted for a stable diff", "[render][perfbench]")
{
	...
	const std::string json = acc.ToJson("g", "c", true);
...
TEST_CASE("PerfBench flags unsupported timestamps with no passes", "[render][perfbench]")
{
	...
	PerfBenchAccumulator acc;
	REQUIRE(acc.Empty());
	const std::string json = acc.ToJson("NoTimestampGPU", "rt-off", false);
